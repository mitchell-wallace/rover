...
This change normalizes Rover around small, deep modules with narrow interfaces, so the command layer reads top-to-bottom as thin adapters and each workflow is a self-contained service that is testable through injected providers. It is a behavior-preserving refactor: no user-visible command, flag, output, or connectivity decision changes.
...
- **Internal architecture**: One God file becomes four focused packages plus a thin command layer. Each workflow is a deep module with a narrow `Service` surface; providers cross package boundaries through small interfaces.
